  ...
  for (size_t i = 0; i < range; i++) {
    ...
    int dup = 0;
    {
      ...
      for (size_t j = 0; j < range; j++) {
        ...
        auto des_norm = norms_[des_pool[j].id];
...
void IndexMips::Build(size_t n, const float *data,
  ...
  unsigned range = parameters.Get<unsigned>("R");
  unsigned big_range = parameters.Get<unsigned>("BR");
...
void IndexMips::Search(const float *query, const float *x, size_t K,
  ...
  std::vector<IpNeighbor> ipset(L + 1);
  std::vector<unsigned> init_ids(L);
  boost::dynamic_bitset<> flags{nd_, 0};
  std::mt19937 rng(rand());

  DistanceInnerProduct *dist_inner = new DistanceInnerProduct();
  
  DistanceFastL2 *dist_fast = new DistanceFastL2();
  GenRandom(rng, init_ids.data(), L, (unsigned)nd_);
  assert(eps_.size() < L);
  ...
  for (unsigned i = 0; i < L; i++) {
    ...
    float ip = dist_inner->compare(data_ + dimension_ * id, query,
                                    (unsigned)dimension_);
    retset[i] = Neighbor(id, dist, true);
    ipset[i] = IpNeighbor(id, ip, true);
    flags[id] = true;
  }
  std::vector<float> nn_path;
  std::vector<float> ip_path;
  std::vector<float> angle_path;
  std::sort(retset.begin(), retset.begin() + L);
  std::sort(ipset.begin(), ipset.begin() + L);
  int k = 0;
  int path_count = 1;
  while (k < (int)L) {
    int nk = L;
    ...
    if (retset[k].flag) {
      ...
      unsigned n = retset[k].id;
      // print path
        float dist = distance_->compare(query, data_ + dimension_ * n,
                                        (unsigned)dimension_);
        float ip = dist_inner->compare(data_ + dimension_ * n, query,
                                        (unsigned)dimension_);
        float query_norm = dist_fast->norm(query, dimension_);
        float data_norm = dist_fast->norm(data_ + dimension_ * n, dimension_);
        float radius = acos(ip / (query_norm * data_norm));
        // std::cout << "path id: " << path_count << " dist: " << dist << " ip: " << ip << " angle: " << radius << std::endl;
        nn_path.push_back(dist);
        ip_path.push_back(ip);
        angle_path.push_back(radius);
        path_count++;
      // end print path
      for (unsigned m = 0; m < final_graph_[n].size(); ++m) {
        unsigned id = final_graph_[n][m];
        ...
        if (dist >= retset[L - 1].distance) continue;
        Neighbor nn(id, dist, true);
        int ipr = InsertIntoIpPool(ipset.data(), L, IpNeighbor(id, ip, true));
        int r = InsertIntoPool(retset.data(), L, nn);
...
void IndexMips::OptimizeGraph(const float *data) {  // use after build or load
...
#pragma omp parallel for
  for(unsigned i=0; i<n_try; i++){
    ...
    while(uncheck_set.size() >0){
      ...
      std::cout << "uncheck_set size:"<< uncheck_set.size() << '\n';
