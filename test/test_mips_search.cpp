...
void read_kmeans(const std::string& filename, int query_count, std::vector<int>& query_cluster, std::vector<std::vector<int>>& init_nodes_clusters, int number_of_clusters) {
    ...
    // Divide init_nodes into clusters of 100
...
void adjust_norm(float* data, unsigned points_num, unsigned dim) {
  for (unsigned i = 0; i < points_num; i++) {
    for (unsigned j = 0; j < dim; j++) {
      data[i * dim + j] *= 10000.0;
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  adjust_norm(query_load, query_num, dim);


  std::string filename = "../output/mnist/nsn.bin";
  ...
  std::cout << "num: " << num / (int)query_num << std::endl;
  std::cout << "Average query time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() / query_num << "ms" << std::endl;
